...
class M_Events extends MY_Model {
	...
	function getUser() {
		...
		if ($this -> input -> post()) {//check if a post was made
			...
			$user = $this -> em -> getRepository('models\Entities\entities_254\E_Events') -> findOneBy(array('email' => $this -> input -> post('email'), 'password' => $this -> input -> post('password')));
	...
	function addRecord() {
		...
		if ($this -> input -> post()) {//check if a post was made
			...
			for ($i = 1; $i <= $this -> noOfInsertsBatch; ++$i) {
				...
				$this -> theForm = new \models\Entities\entities_254\E_Events();
